...
int main(){
    ...
    int interview_num = m*1.5 , count_ = 0;
    int interview_score = v[interview_num-1].score;
    for (size_t i = 0; i < n; i++)
    {
        if (v[i].score >= interview_score)
        {
            count_ ++;
        }
        else break;
    }
    cout << interview_score<<' '<<count_<<endl;
